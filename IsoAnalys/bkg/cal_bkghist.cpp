	 ...
	 private:
		 ...
		 void ProcessEvents() {
			 ...
			 for(Long64_t entry = 0; entry < tree->GetEntries(); ++entry) {
				 ...
				 if(mtrpar[1]!=63 && mtrpar[1]!=64 && mtrpar[1]!=114) continue;
						 ...
						 for(int sel = 0; sel < 2; ++sel) {
							 ...
							 for(int bkg = 0; bkg < 3; ++bkg) {
								 if(BkgStudyCut[sel][bkg][det] == 1) {
									 ...
									 if(mparc[0] == 4 && mtrpar[1] == isoCode[iso]) {  // mc truth info
										 if(generatedEk > 0) {
		 ...
		 void SaveResults() {
			 std::unique_ptr<TFile> outFile(TFile::Open((outputDir_ + "/" + nucleus_.name + 
							 std::to_string(currentMass_) + "_bkg_analysis_NewTOI.root").c_str(), "RECREATE"));
