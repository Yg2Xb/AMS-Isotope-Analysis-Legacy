 ...
 ***********************************************************/
 ...
 void ProcessTreeWithMass(TFile* file, const char* treeName, const char* outputFileName, 
     ...
     TH1F *hB11toBe10 = new TH1F("hB11toBe10","Boron11MCtoBe10 in Layer2;events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
     TH1F *hB11 = new TH1F("hB11","B11 in Layer1; events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
     ...
     for (Long64_t i = 0; i < nEntries; ++i) {
         ...
         if(TOFEk > 2 && (NaFEk < 0.8 || NaFEk > 4) && AGLEk < 2.5) continue;
         ...
         bool Qcut = (config.charge == -1) ? 
                     ...
                     : (isMC ? (tk_qin[0][2] > 3.5 && tk_qin[0][2] < 5.5 && q_l1_unbiased > 4.5 && q_l1_unbiased < 5.5) : (tk_qin[0][2] > 3.45 && tk_qin[0][2] < 4.45 && q_l1_unbiased > 5.0 && q_l1_unbiased < 5.4));
         if(!Qcut) continue;
         ...
         bool processTOF = (cutStatus & BKG_TOF_MASK) == BKG_TOF_MASK && isValidBeta(TOFBeta) && TOFEk > 0 && TOFEk < 1.17; 
         bool processNaF = (cutStatus & BKG_RICH_MASK) == BKG_RICH_MASK && rich_NaF && isValidBeta(NaFBeta) && NaFEk >= 1.17 && NaFEk < 3.32; 
         bool processAGL = (cutStatus & BKG_RICH_MASK) == BKG_RICH_MASK && !rich_NaF && isValidBeta(AGLBeta) && AGLEk >= 3.32; 
         ...
         if (isMC) {
             ...
             if (processNaF) {
                 ...
                 hB11->Fill(betaToKineticEnergy(smearBeta));
                 if(mtrpar[1]==114 && tk_qin[0][2] > 3.5 && tk_qin[0][2] < 4.5) hB11toBe10->Fill(betaToKineticEnergy(smearBeta));
                 /*
 ...
 void FillTempHist_test(const std::string& NucName) {
