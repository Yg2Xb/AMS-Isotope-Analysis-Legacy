...
void ProcessTreeWithMass(TFile* file, const char* treeName, const char* outputFileName, 
    ...
    // Setup branches
    double L1InnerRig = -1, TOFBeta = -1, NaFBeta = -1, AGLBeta = -1, TOFEk = -1, NaFEk = -1, AGLEk = -1;
    float QTrkInner[3], trk_ql1, trk_ql1_unbias, trk_ql2, rich_q[2], tof_ql[4];
    bool rich_NaF = false;
    unsigned int cutStatus = 0;
    
    float mmom = 0; double generatedRig = -1; int mtrpar[9];
    // MC branch
    if (isMC) {
    ...
    // common branch
    tree->SetBranchAddress("QTrkInner", QTrkInner);
    tree->SetBranchAddress("rich_q", rich_q);
    tree->SetBranchAddress("tof_ql", tof_ql);
    tree->SetBranchAddress("trk_ql1", &trk_ql1);
    tree->SetBranchAddress("trk_ql1_unbias", &trk_ql1_unbias);
    tree->SetBranchAddress("trk_ql2", &trk_ql2);
    tree->SetBranchAddress("L1InnerRig", &L1InnerRig);
    ...
    //bin initial
    std::vector<std::array<double, 31>> all_ek_bins;
    std::vector<std::vector<double>> beta_bins;
    for (int mass : config.masses) {
        std::cout << mass << std::endl;
        const auto& ek_bins = Binning::NarrowBins;
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        double q_inner = QTrkInner[0];
        double q_rich = sqrt(rich_q[0]);
        if(q_inner > 5.5) continue;
        
        bool basiccut = (cutStatus & BKG_BASIC_MASK) == BKG_BASIC_MASK; 
        if(!basiccut) continue;
        bool L1InnerCut = (cutStatus & L1INNER_MASK) == L1INNER_MASK; 
        bool unbL1InnerCut = (cutStatus & UNBL1INNER_MASK) == UNBL1INNER_MASK; 
        
        
        bool Qcut = (config.charge == 4) ?
                    (unbL1InnerCut)  //if Be, L1Inner Normal iss and mc temp
                    : (false ? (q_inner > 3.5 && q_inner < 5.5 && trk_ql1 > 4.8 && trk_ql1 < 5.5) //if Bor, L1Inner Bkg Select
                            : (q_inner > 3.45 && q_inner < 5.45 && trk_ql1_unbias > 5.0 && trk_ql1_unbias < 5.4)); 
        //select L1 Boron(with Be C contamination) + Inner Be, B frag. to Be + Be contami. sample
        if(!Qcut) continue;

        bool TargetQcut = (config.charge == 4) ? true : q_inner > 3.45 && q_inner < 4.45;
        ...
        unsigned int RICH_MASK = (config.charge == 4) ? NORM_RICH_MASK : BKG_RICH_MASK;
        bool detectorValid[DETECTOR_COUNT] = {
            //<0 if no det. beta
            (cutStatus & BKG_TOF_MASK) == BKG_TOF_MASK && isValidBeta(TOFBeta),
            (cutStatus & RICH_MASK) == RICH_MASK && isValidBeta(NaFBeta),
            (cutStatus & RICH_MASK) == RICH_MASK && isValidBeta(AGLBeta)
        };
        ...
        if (isMC) {
            ...
            for (int det = 0; det < DETECTOR_COUNT; det++) {
                ...
                if (TempFitTargetQcut && isValidBeta(TunedBeta)) {
                    for (int j = 10; j < 11; ++j) {
                        ...
                        auto result = calculateMass(TunedBeta, alpha, L1InnerRig, charge); //change!!
            ...
            for (size_t j = 0; j < 1; ++j) { //config.masses.size
                ...
                for (int det = 0; det < DETECTOR_COUNT; det++) {
                    ...
                    int useA = (config.charge == 4) ? 7 : 10;
                    bool cutoffcut = (betaBin >= 0) ? isBeyondCutoff(mass_beta_bins[betaBin], cutOffRig, SAFETY_FACTORS[det], config.charge, useA, isMC) : false;
                    
                    if (cutoffcut) {
                        //no truth info in iss, no source[det][0]
                        if (j == 0) bgHist.SourceCounts[det][1]->Fill(data.ek);//total counts pass Qcut and cutoff cut

                        if(!TargetQcut) continue;
                        if (j == 0) bgHist.FragNucCounts[det]->Fill(data.ek);//counts pass inner frag q cut

                        if(!TempFitTargetQcut) continue; // extra cut for mass tempfit
                        auto result = calculateMass(data.beta, 1.0, L1InnerRig, charge);
...
void BuildTempHist(const std::string& isotype) {
    ...
    try {
        ...
        // process iss
        {
            ...
            //std::string issFile = baseDir + "BtoBeBkgTest" + ".root";
            std::string issFile = baseDir + "Bor_BeToC" + ".root";
            std::string outFile = baseDir + config.dataName + (config.charge == 4 ? "_temp_narrow_unbL1_510.root" : "_temp_narrow_bkg_unbL1_SDIATBeBcut.root");
        ...
        // process each mc
        for (int mass : config.masses) {
            //if(mass!=11) continue; //only B11
            std::string mcFile = baseDir + config.mcPrefix + std::to_string(mass) + "_bkg.root";
            std::string outFile = baseDir + config.mcPrefix + std::to_string(mass) + (config.charge == 4 ? "_temp_narrow_unbL1.root" : "_temp_narrow_frag_unbL1_SDIATBeBcut.root");
