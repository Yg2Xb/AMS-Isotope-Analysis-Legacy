...
void ProcessTreeWithMass(TFile* file, const char* treeName, const char* outputFileName, 
    ...
    float tk_qin[2][3], tk_exqln[2][2][3], rich_q[2], tof_ql[4];
    bool rich_NaF = false;
    ...
    std::vector<std::unique_ptr<TH2D>> h2_inv_ToFMass;
    std::vector<std::unique_ptr<TH2D>> h2_inv_NaFMass;
    std::vector<std::unique_ptr<TH2D>> h2_inv_AGLMass;
    ...
    //
    std::vector<std::unique_ptr<TH1D>> h_SelTotalNumber;
    std::vector<std::unique_ptr<TH1D>> h_MCBoronToBerIso;
    ...
    std::vector<std::vector<double>> beta_bins;
        for (int mass : config.masses) {
            cout<<mass<<endl;
            const auto& ek_bins = Binning::WideBins;//getKineticEnergyBins(config.charge, mass);// 
    ...
    if (isMC) {
        ...
        for (int j = 10; j < 11 ; ++j) {
            for (size_t k = 0; k < config.masses.size(); ++k) {
                ...
                // ToF直方图
                h2_inv_ToFMass.push_back(std::make_unique<TH2D>(
                    Form("h2_inv_ToFMass_mass%d_bin%d_alpha%d", processMass, mass, j),
                    "inv_ToFMass", 200, 0.0, 0.5, ek_bins.size()-1, ek_bins.data()
                ));
                
                // NaF直方图
                h2_inv_NaFMass.push_back(std::make_unique<TH2D>(
                    Form("h2_inv_NaFMass_mass%d_bin%d_alpha%d", processMass, mass, j),
                    "inv_NaFMass", 200, 0.0, 0.5, ek_bins.size()-1, ek_bins.data()
                ));
                
                // AGL直方图
                h2_inv_AGLMass.push_back(std::make_unique<TH2D>(
                    Form("h2_inv_AGLMass_mass%d_bin%d_alpha%d", processMass, mass, j),
                    "inv_AGLMass", 200, 0.0, 0.5, ek_bins.size()-1, ek_bins.data()
                ));
            }
        }
    ...
    TH1F *hB11toBe10_TOF = new TH1F("hB11toBe10_TOF","TOF Boron11MCtoBe10 in Layer2;events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    TH1F *hB11toBe10_NaF = new TH1F("hB11toBe10_NaF","NaF Boron11MCtoBe10 in Layer2;events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    TH1F *hB11toBe10_AGL = new TH1F("hB11toBe10_AGL","AGL Boron11MCtoBe10 in Layer2;events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    TH1F *hB11_TOF = new TH1F("hB11_TOF","TOF B11 in Layer1; events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    TH1F *hB11_NaF = new TH1F("hB11_NaF","NaF B11 in Layer1; events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    TH1F *hB11_AGL = new TH1F("hB11_AGL","AGL B11 in Layer1; events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    TH1F *hB11toBe10_B11_ratio = new TH1F("hB11toBe10_B11_ratio","Boron11MCtoBe10 in Layer2 / B11 in Layer1;events; E_{k}/n", Binning::WideBins.size()-1, Binning::WideBins.data());
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        //if(TOFEk > 2 && (NaFEk < 0.6 || NaFEk > 4) && AGLEk < 2.5) continue;

        double q_l1_unbiased = tk_exqln[0][0][2];
        double richQ = sqrt(rich_q[0]);
        double lowtofq = (tof_ql[2]+tof_ql[3])/2.;
        //if(tof_ql[2]== 0 || tof_ql[3] == 0) lowtofq = lowtofq*2;
        bool Qcut = (config.charge == -1) ? 
                    (tk_qin[0][2] > 4.5 && tk_qin[0][2] < 5.5 && q_l1_unbiased > 4.5 && q_l1_unbiased < 5.5) 
                    : (isMC ? (tk_qin[0][2] > 3.5 && tk_qin[0][2] < 5.5 && q_l1_unbiased > 4.5 && q_l1_unbiased < 5.5) : (tk_qin[0][2] > 3.5 && tk_qin[0][2] < 5.5 && q_l1_unbiased > 4.5 && q_l1_unbiased < 5.5));
        if(!Qcut) continue;
        //if(basiccut2 != (cutStatus & (1<<0))) cout<<"No1111111"<<endl;
        ...
        if (isMC) {
            ...
            //weight = calculateWeight(mmom, config.charge, isMC);
        }
        
        // Process conditions using named constants
        bool processTOF = (cutStatus & BKG_TOF_MASK) == BKG_TOF_MASK && isValidBeta(TOFBeta); 
        bool processNaF = (cutStatus & BKG_RICH_MASK) == BKG_RICH_MASK && rich_NaF && isValidBeta(NaFBeta); 
        bool processAGL = (cutStatus & BKG_RICH_MASK) == BKG_RICH_MASK && !rich_NaF && isValidBeta(AGLBeta); 
        
        
        // Fill histograms
        int isotopeCode = -1;
        if(config.charge == 4){
            isotopeCode = (processMass == 7) ? 63 : (processMass == 9) ? 64 : (processMass == 10) ? 114 : -1; 
        }
        else{
            isotopeCode = (processMass == 10) ? 65 : (processMass == 11) ? 66 : -1;  
        }
        

        if (isMC) {
            //if(mtrpar[1]==114) cout<<tk_qin[0][2]<<endl;
            //select below L2 pure Isotope
            bool pureMC = (mtrpar[1] == isotopeCode);
            //if(!pureMC) continue;

            if (processTOF) { 
                if(TOFEk > 0 && TOFEk < 1.17){
                    hB11->Fill(TOFEk);
                    if(mtrpar[1]==114 && tk_qin[0][2] > 3.5 && tk_qin[0][2] < 4.5) hB11toBe10->Fill(TOFEk);
                }
                ...
                for (int j = 0; j < 21; ++j) {
                    ...
                    if (result.isValid()) {
                        for (size_t k = 0; k < config.masses.size(); ++k) {
                            ...
                            if(pureMC)
                            {
                                h2_inv_ToFMass[hist_idx]->Fill(result.invMass, TOFEk, weight);
                            }
                        }
                    }
                }
            ...
            if (processNaF) {
                double smearBeta = GetSmearRichBeta(config.charge, NaFBeta, true);
                hB11->Fill(NaFEk);
                if(mtrpar[1]==114 && tk_qin[0][2] > 3.5 && tk_qin[0][2] < 4.5) hB11toBe10->Fill(NaFEk);
                    ...
                    for (int j = 0; j < 21; ++j) {
                        ...
                        if (result.isValid()) {
                            for (size_t k = 0; k < config.masses.size(); ++k) {
                                ...
                                if(pureMC)
                                {
                                    h2_inv_NaFMass[hist_idx]->Fill(result.invMass, NaFEk, weight);
                                }
                            }
                        }
                    }
            ...
            if (processAGL) {
                double smearBeta = GetSmearRichBeta(config.charge, AGLBeta, false);
                hB11->Fill(AGLEk);
                if(mtrpar[1]==114 && tk_qin[0][2] > 3.5 && tk_qin[0][2] < 4.5) hB11toBe10->Fill(AGLEk);
                    ...
                    for (int j = 0; j < 21; ++j) {
                        ...
                        if (result.isValid()) {
                            for (size_t k = 0; k < config.masses.size(); ++k) {
                                ...
                                if(pureMC)
                                {
                                    h2_inv_AGLMass[hist_idx]->Fill(result.invMass, AGLEk, weight);
                                }  
                            }
                        }
                    }
            ...
            for (size_t j = 0; j < config.masses.size(); ++j) {
                ...
                if (processTOF) {
                    ...
                    //if(Qcut){
                    if(TOFBin >= 0 && isBeyondCutoff(mass_beta_bins[TOFBin], cutOffRig, SAFETY_TOF, 5, 10, isMC)){
                ...
                if (processNaF) {
                    ...
                    //if(Qcut){
                    if(NaFBin >= 0 && isBeyondCutoff(mass_beta_bins[NaFBin], cutOffRig, SAFETY_NAF, 5, 10, isMC)){
                ...
                if (processAGL) {
                    ...
                    //if(Qcut){
                    if(AGLBin >= 0 && isBeyondCutoff(mass_beta_bins[AGLBin], cutOffRig, SAFETY_AGL, 5, 10, isMC)){
    ...
    hB11->Sumw2();
    hB11toBe10->Sumw2();
    hB11toBe10_B11_ratio->Divide(hB11toBe10,hB11);
    ...
    for (size_t i = 0; i < h_inv_ToFMass.size(); ++i) {
        ...
        if(isMC){
            h2_inv_ToFMass[i]->Write();
            h2_inv_NaFMass[i]->Write();
            h2_inv_AGLMass[i]->Write(); 
        }
    }
    hToF->Write();
    hNaF->Write();
...
void BuildTempHist(const std::string& isotype) {
    ...
    try {
        ...
        std::string outFile = baseDir + config.dataName + "_temp_wide_bkgBBeSDIAT_rq_sf.root";
        ...
        for (int mass : config.masses) {
            ...
            std::string outFile = baseDir + config.mcPrefix + std::to_string(mass) + "_B11toBe10Evnets.root";
