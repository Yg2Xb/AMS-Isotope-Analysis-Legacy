...
void HistTempFit(const std::string& isotype, const std::string& suffix,
	...
	// 打开数据文件
	TFile* f_data = TFile::Open((inputDir + "/" + config.dataName + "_temp.root").c_str());
	cout << f_data->GetName() << endl;

	// 打开MC文件
	std::vector<TFile*> f_mc;
	for (int mass : config.masses) {
		f_mc.push_back(TFile::Open((inputDir + "/" + config.name + 
						std::to_string(mass) + "_temp.root").c_str()));
	}
	...
	canvas->Print((outputDir + "/MassTF_" + config.name + "_" + suffix + 
				"_Use" + std::to_string(UseMass) + ".pdf[").c_str());

	// 获取能量区间
	const auto& ek_bins = getKineticEnergyBins(config.charge, UseMass);
	for(int b = 0; b < ek_bins.size()-1; b++){
		cout<<ek_bins[b]<<endl;
	}
	// 创建用于存储所有探测器结果的ROOT文件
	TFile* output_file = TFile::Open(
			Form("%s/results_%s_%s_mass%d.root", outputDir.c_str(),
	...
	for (int idet = 0; idet < 3; idet++) {
		...
		double alpha_min = 0.98, alpha_step = 0.001, alpha_max = 1.02;

		// 创建结果储存直方图
		TH2F* h_chi2_ndf = new TH2F(Form("h_chi2_ndf_%s", DetName[idet]), 
				Form("%s Chi2/NDF for each Ek bin", DetName[idet]), 
				ek_bins.size() - 1, ek_bins.data(), 41, alpha_min - alpha_step/2, 
		...
		// 获取数据直方图
		TH2F* data_hist_2d = (TH2F*)f_data->Get(Form("h_inv_%s_%sUseMass%d",
					IsoFitConstants::x_titles[idet], config.dataName.c_str(), UseMass));

		// 能量bin循环
		for (int ibin = 6; ibin <= 38; ibin++) {
			// 检查bin是否在探测器范围内
			if (idet == 0 && ibin >= config.detRanges[0]) continue;
			if (idet == 1 && (ibin < 6 || ibin > 27)) continue;
			if (idet == 2 && ibin < config.detRanges[1]) continue;

			// 初始化本bin的最佳拟合记录
			FitCache fit_cache;
			double chi2_min = 5000;
			int best_alpha_index = -1;
			// alpha循环
			for (int j = 0; j < 41; j = j + 1) {
				double alpha = alpha_min + j * alpha_step;

				// 获取MC直方图
				std::vector<TH2F*> mc_hist_2d;
				for (int idm = 0; idm < config.masses.size(); idm++) {
					mc_hist_2d.push_back((TH2F*)f_mc[idm]->Get(Form("h_inv_%s_mass%d_alpha_%d", 
									IsoFitConstants::x_titles[idet], config.masses[idm], j)));
				}
				...
				// 投影数据直方图
				TH1D* data_hist = data_hist_2d->ProjectionX(
						Form("data_hist_bin%d_alpha%d", ibin, j), ibin, ibin);

				// 投影并平滑MC直方图
				std::vector<TH1D*> mc_hists;
				for (size_t i = 0; i < mc_hist_2d.size(); ++i) {
					TH1D* mc_hist = mc_hist_2d[i]->ProjectionX(
							Form("%s%d_hist_bin%d_alpha%d", config.name.c_str(),
								config.masses[i], ibin, j), ibin, ibin);
					mc_hist->Smooth(1);
				...
				// 执行拟合
				cout<<"detector "<<DetName[idet]<<" begin fit: ek/n="<<ek_bins[ibin-1]<<" ,alpha= "<<alpha<<endl;
				RooFitResult* fit_res = model.fitTo(data, RooFit::Save(), RooFit::SumW2Error(kTRUE), RooFit::Verbose(kFALSE));
				cout<<"detector "<<DetName[idet]<<" END fit: ek/n="<<ek_bins[ibin-1]<<" ,alpha= "<<alpha<<endl;
				// 计算chi2
                ...
                const std::vector<int> lineColors = {kBlue, kGreen+2, kOrange+1, kViolet};
                for (size_t i = 0; i < pdfs.size(); ++i) {
				...
				double chi2_ndf = (ndf > 0 && fit_res->status() == 0) ?
					chi2/ndf : 5000;

				// 存储当前alpha的拟合结果到二维直方图
				h_chi2_ndf->SetBinContent(ibin, j + 1, chi2_ndf);
				...
				double signal_efficiency = result.efficiency;
				double upperBound = result.upperBound;
				...
				if (chi2_ndf < chi2_min) {
					fit_cache.UpdateCache(ibin, chi2_ndf, alpha, fit_res, fractions,
							frame, mc_hists, data_hist, significance, 
							signal_efficiency, upperBound, N_sig, N_tot, n_entries);
					chi2_min = chi2_ndf;
					best_alpha_index = j;
				}
			...
			if (best_fit && best_fit->is_valid) {
				...
				// 添加图例
				TLegend* legend = new TLegend(0.18, 0.63, 0.45, 0.85);
				...
				// 添加信息文本框
				TPaveText* pt = new TPaveText(0.71, 0.63, 0.85, 0.85, "NDC");
				...
				setupPullPlot(pullGraph, IsoFitConstants::x_titles[idet], config.fitRangeLow[idet], config.fitRangeUp[idet]);
...
void HistTempFit_hist() {
	HistTempFit("Be", "DiffBin_Be", "/eos/ams/user/z/zuhao/yanzx/Isotope/NewData",
