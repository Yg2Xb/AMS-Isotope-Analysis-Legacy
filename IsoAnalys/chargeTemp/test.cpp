...
public:
    ...
    void plotTempComparison(const vector<TH1*>& hists, const vector<int>& colors,
        ...
        for (size_t i = 0; i < hists.size(); i++) {
            ...
            string baseName = "splinefit_" + labels[i] + "_" + to_string(i);
...
void saveSplineFits(const string& inputRootFile) {
    size_t lastSlash = inputRootFile.find_last_of("/");
    string directory = (lastSlash != string::npos) ? inputRootFile.substr(0, lastSlash + 1) : "";
    string outputFile = directory + "allFitHistSplineSmooth_0p8.root";
    ...
    for (const auto& element : config.elements) {
        for (const auto& temp : config.temps) {
            for (const auto& param : config.paramList) {
                ...
                if (splineFit) {
                    string splineName = element + "_Combined_" + temp + "_" + param.first + "_" + param.second + "_SplineSmooth";
