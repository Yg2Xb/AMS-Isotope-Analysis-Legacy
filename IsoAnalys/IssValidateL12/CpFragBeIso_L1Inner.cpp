...
void CpFragBeIso() {
    ...
    // 文件路径
    TString issFile = "/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/IssValidateL12/ISS_FragmentBer_MassFitAnalysis_L1Inner_510cutoff_perfectQfit.root";
    TString mcB11File = "/eos/ams/user/z/zuhao/yanzx/Isotope/NewData/B11_temp_wide_bkg_frag_L1Inner.root";
    TString mcB10File = "/eos/ams/user/z/zuhao/yanzx/Isotope/NewData/B10_temp_wide_bkg_frag_L1Inner.root";
    ...
    for (size_t ibe=0; ibe<beNames.size(); ++ibe) {
        ...
        for (int bin=1; bin<=hCombined->GetNbinsX(); ++bin) {
            ...
            if(!h_BeFromB11 || !h_BeFromB10) continue;
    ...
    for(int i=0; i<3; ++i) {
        ...
        legend->Draw();
        c->SetGrid();
        c->SaveAs(TString::Format("/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/IssValidateL12/Be%d_ISS_MC_perfectQfit_ori_37.png", beNumbers[i]));
        if(i==2)
        {
            hISS[0]->Draw("pz");
            ...
            legend->Draw();
            c->SetGrid();
            c->SaveAs(TString::Format("/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/IssValidateL12/Be79_ISS_perfectQfit_ori_37.png", beNumbers[i]));
        }
        delete c;
    }
