...
void calculateFragmentedBeryllium(bool isISS) {
    ...
    //bkgBBeFinal
    const char *FileSuffix = isISS ? "BtoBe_510cutoff_L1Inner" : "MCB11toBeIso";
    cout<<FileSuffix<<endl;
    const char *FilePrefix = isISS ? "ISS" : "MCB11";
    const char *CountsFileName = isISS ? "Bor_temp_wide_bkg_L1Inner510cutoff" : "B11_temp_wide_bkg_frag";
    ...
    auto normalBeFitFile   = openFile("/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/TempFit/MassTF_Be_wide_510cutoff_L1Inner_Use7.root");
    auto CountsFile        = openFile(Form("/eos/ams/user/z/zuhao/yanzx/Isotope/NewData/%s.root", CountsFileName));
    auto Be_ratioFile      = openFile("/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/chargeTempFit/chargeFit_B_EGE_detfuncT_wide.root");
    auto B_ratioFile       = openFile("/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/chargeTempFit/chargeFit_B_EGE_detfuncT_wide.root");

    auto B11IsoFracFile = openFile("/eos/ams/user/z/zetong/Boron_isotope/pre/B11f1.root"); 
    
    auto outputFile = openFile(Form("/eos/ams/user/z/zuhao/yanzx/Isotope/IsoResults/IssValidateL12/%s_FragmentBer_MassFitAnalysis_L1Inner_510cutoff_perfectQfit.root",FilePrefix), "RECREATE");
    ...
    for (int bin_idx = 1; bin_idx <= nBins; ++bin_idx) {
        ...
        double raw_FracNuc_val = inputFragNucCounts_HistMap[detName]->GetBinContent(bin_idx);
        ValueWithError raw_FragNucCount_vw(raw_FracNuc_val, (raw_FracNuc_val >=0 ? std::sqrt(raw_FracNuc_val) : 0.0) );

        double raw_L1Souce_val = inputL1SourceCounts_HistMap[detName]->GetBinContent(bin_idx);
        ...
        if(h_be_ratio_src->GetBinCenter(bin_idx+1) != binCenter) cout<<"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<<endl;
